...
namespace ImageTemplate
{
    ...
    public class ImageOperations
    {
        ...
        public struct Edge
        {
            ...
            public int To;
            //public byte weight;
            public byte Red_Weight;
            public byte green_Weight;
            public byte blue_Weight;

            public Edge(int from, int to, byte Red_weight, byte Green_weight, byte Blue_weight)
            {
                From = from;
                To = to;
                //this.weight = weight;
        ...
        public static (DisjointSet, PixelNode[,], Edge[], /*Edge[], Edge[],*/ DisjointSet, DisjointSet, DisjointSet) BuildGraph(RGBPixel[,] image)
        {
            ...
            var rEdges = new Edge[height * width * 4];
            var gEdges = new Edge[height * width * 4];
            var bEdges = new Edge[height * width * 4];
            ...
            nodeMap[0, 0] = new PixelNode(0, 0, image[0, 0], 0);

            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    Rset.make_set(nodeMap[y, x].id, y, x);
                    Gset.make_set(nodeMap[y, x].id, y, x);
                    Bset.make_set(nodeMap[y, x].id, y, x);
                    regionSet.make_set(nodeMap[y, x].id, y, x);
                    int[,] directions = {
                                    {0, 1},
                                    {1, 1},
                                    {1, 0},
                    ...
                    for (int i = 0; i < directions.GetLength(0); i++)
                    {
                        ...
                        {
                            PixelNode neighbor;
                            if (nodeMap[ny, nx].id != 0)
                            {
                                neighbor = new PixelNode(nx, ny, image[ny, nx], nodeMap[ny, nx].id);
            ...
            return (regionSet, nodeMap, rEdges,/* gEdges, bEdges,*/ Rset, Gset, Bset);
        }

        public static DisjointSet components(DisjointSet regionSet, PixelNode[,] nodeMap, int height, int width, int K, Edge[] Rdges,/* Edge[] Gdges, Edge[] Bdges,*/ DisjointSet Rset, DisjointSet Gset, DisjointSet Bset)
        {
            /*List<Edge> Gdges = Rdges;
            List<Edge> Bdges = Rdges;*/

            //Rdges.Sort((a, b) => a.weight.CompareTo(b.weight));
            var Gdges = new Edge[width * height * 4];
            var Bdges = new Edge[width * height * 4];
            ...
            //List<Edge> redges = new List<Edge>();
            ...
            for (int i = 0; i < height * width * 4; i++)
            {
                ...
                if (rootF != rootT)
                {
                    ...
                    if (Mint >= e.Red_Weight)
                    {
                        //redges.Add(e);
                        //Console.WriteLine("here " + K);
                        Rset.Union(e.From, e.To);
                        Rset.it[Rset.Find(rootF)] = e.Red_Weight;
                ...
                rootF = Gset.Find(e.From);
                rootT = Gset.Find(e.To);
                ...
                if (rootF != Gset.Find(e.To))
                {
                    ...
                    if (Mint >= e.green_Weight)
                    {
                        //gedges.Add(e);
                        Gset.Union(e.From, e.To);

                        Gset.it[Gset.Find(rootF)] = e.green_Weight;
                ...
                e = Bdges[i];
                rootF = Bset.Find(e.From);
                rootT = Bset.Find(e.To);
                ...
                if (rootF != rootT) //&& Rset.Find(e.From.id) == Rset.Find(e.To.id))
                {
                    ...
                    double Mint = Math.Min(itT, itF);
                    if (Mint >= e.blue_Weight)
                    {
                        //bedges.Add(e);
                        Bset.Union(e.From, e.To);
                        Bset.it[Bset.Find(rootF)] = e.blue_Weight;
        ...
        public static void WriteDisjointSetsToDesktop(PixelNode[,] nodeMap, DisjointSet set, int width, int height)
        {
